...
senato22[COMUNE == "VALSAMOGGIA", #| COMUNE == "PIACENZA" | COMUNE == "FIRENZE" ,
       .(VOTICANDIDATO,  VOTILISTA, DESCRLISTA, COGNOME, COMUNE,
         COLLPLURI, COLLUNINOM)]


# GET RESULTS BY COMUNE
camera22[COMUNE == "NAPOLI", #| COMUNE == "PIACENZA" | COMUNE == "FIRENZE" ,
       .( 100*( VOTICANDIDATO - sum(VOTILISTA) )/ VOTICANDIDATO , VOTICANDIDATO
          ,DESCRLISTA),
       by = COGNOME]

# Get list total votes
c = camera22[, .(sum = sum(VOTILISTA)), by = .(DESCRLISTA )][sum > 10e5] [order(-sum)]
...
names(camera22)
camera22[, length(unique(COMUNE))]
camera22[, length(unique(COLLUNINOM))]
...
camera22[, .(REGION = str_extract(`CIRC-REG`, '\\w*'))][,unique(REGION)]
senato22[, unique(`CIRC-REG`)]
