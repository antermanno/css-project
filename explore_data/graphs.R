...
data_cor = inner_join(data_cor, gtrend_reg)
...
full_tbl = inner_join(camera18, gtrend_reg)
...
diff |>
  ggplot(aes(x = scale(delta), y = RCSR_all_time, colour = REGION))+
  geom_point()+
  geom_abline()+
  theme_bw()
